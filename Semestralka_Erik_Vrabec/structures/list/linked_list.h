...
namespace structures
{
    ...
    inline LinkedList<T>::LinkedList() :
        size_(0),
        first_(nullptr),
        last_(nullptr)       //dynamick· ötrukt˙ra, nevytv·rame niË dopredu
    {
    }
    ...
    template<typename T>
    inline void LinkedList<T>::add(const T& data)  //vloûenie na zaËiatok
    {
        size_++;
    ...
    inline void LinkedList<T>::insert(const T& data, int index)
    {
        if (index == size_) {
            ...
            if (index == 0) {
                ...
                LinkedListItem<T>* prevLLI = getItemAtIndex(index - 1); //uloûÌme si predchodcu
    ...
    {
        //maûeme od first_ po last_
        // prid·me si current, ukazovateæ za first_
    ...
    inline LinkedListItem<T>* LinkedList<T>::getItemAtIndex(int index)
    {
        ...
        if (index == size_ - 1) {
            ...
            for (int i = 0; i < index; i++) {
                current = current->getNext();  //ideme po zreùazenom zozname ku tomu Ëo chceme
            }
        }
        return current;
    }
    ...
    {
        position_ = nullptr;        // delete nie, lebo zmaûeme LinkedListItem
    }
